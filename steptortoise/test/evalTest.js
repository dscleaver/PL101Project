  ...
  suite('Expressions', function() {
    ...
    suite('function calls', function() {
      test('looked up in environment', function() {
        assert.deepEqual(
          evalString("f();", { bindings: { f: function(cont) { return { tag: "thunk", func: cont, args: [7] }; } }, outer: null }),
          7
        );
      });
      test('pass args to function', function() {
        assert.deepEqual(
          evalString("f(1 + 2, 3, 4 * 6);", { bindings: { f: function(cont, x, y, z) { return { tag: "thunk", func: cont, args: [ [x, y, z] ]}; } }, outer: null }),
          [ 3, 3, 24 ]
        );
      });
    });
  ...
  suite('Statements', function() {
    ...
    suite('Define', function() {
      ...
      test('defined function evals statements in new environment when called', function() {
        var env = { bindings: {}, outer: null };
        assert.deepEqual(
          evalString("define f() { var x; x := 1; x; } f();", env),
          1
        );
